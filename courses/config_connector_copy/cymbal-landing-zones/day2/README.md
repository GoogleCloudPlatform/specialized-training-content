...
> `jwd-cc-s1` / `us-central1` tokens. `prep.sh` already rewrote
...
```bash
bq show --schema --format=prettyjson "jwd-cc-s1:clickstream.events"
```

**3. The live bucket lifecycle age is now `60`:**

```bash
gcloud storage buckets describe "gs://jwd-cc-s1-clickstream-raw" \
