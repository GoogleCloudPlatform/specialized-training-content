...
```yaml
metadata:
  name: my-database      # ← also becomes the Firestore database's ID in GCP
```

**When `metadata.name` can't be the GCP name.** Kubernetes object names are
restricted — lowercase RFC-1123 (letters, digits, `-`, `.`), max 253 chars.
Many GCP resources allow names that violate those rules: uppercase letters,
...
spec:
  resourceID: "My_Actual_GCP-Name"  # the real GCP identifier (preferred field)
```
...
(often) on the GCP resource itself. It's worth knowing which is which.
...
#### Labels that become labels on the GCP resource

For most labelable resources, KCC **copies `metadata.labels` onto the actual GCP
resource's labels**. So the `team: platform` above shows up as a label on the
Cloud Storage bucket in GCP, not just on the K8s object. The propagation runs
through
...
- **Prefixed labels are stripped.** Any label key containing a `/`—the
  ...
  labels are Kubernetes-domain metadata that GCP has no use for.
- **KCC adds its own management label.** Every GCP resource KCC manages gets
  ...
  it's how you can tell in the GCP console which resources are under Config
  Connector's control.
