...
> **Future migration note:** The Data Agent and Intervention Agent are currently deployed as Cloud Run services with A2A endpoints. The long-term goal is to migrate both to Agent Engine for unified lifecycle management, telemetry, and publishing. See section 11 for details.
...
- Reference documents are authored as markdown in `reference_docs/markdown/`. Pre-generated PDFs are checked in under `reference_docs/pdf/` and uploaded to a GCS bucket by `setup.sh`. To regenerate PDFs from markdown source, run `setup/convert_md_to_pdf.sh` manually (requires Node.js/npm for `md-to-pdf`)
...
- **Datastore ID:** `cymbal-meet-docs` (location: `global`)
...
For the lab, use a **custom service account** passed at deployment time via the `adk deploy agent_engine` CLI. This is explicit, teachable, and lets students see exactly what permissions are granted:
...
# .env.deploy — runtime env vars for the deployed agent
...
**Note:** As of ADK v1.16.0+, `VertexAiSearchTool` supports `bypass_multi_tools_limit=True`, allowing it to coexist with other tools on the same agent. The Intervention Agent can use `VertexAiSearchTool` alongside PDF generation and GCS MCP tools directly — no sub-agent needed.
...
**Agent connection** — uses ADK's `auth_scheme`/`auth_credential` pipeline for automatic token refresh (same pattern as the BigQuery MCP connection in the Data Agent):
```python
...
GCS_MCP_URL = os.getenv("GCS_MCP_URL")  # e.g., https://gcs-mcp-server-HASH.us-central1.run.app/mcp
GCS_SCOPES = ["https://www.googleapis.com/auth/cloud-platform"]

gcs_mcp_tools = McpToolset(
    connection_params=StreamableHTTPConnectionParams(url=GCS_MCP_URL),
    auth_scheme=OAuth2(
        flows=OAuthFlows(
...
│   ├── convert_md_to_pdf.sh                # Converts reference doc markdown to PDF (npx md-to-pdf)
│   ├── upload_reference_docs.py            # Uploads PDFs to GCS
...
│   └── pdf/                                # Converted PDFs (uploaded to GCS for Vertex AI Search)
│       └── *.pdf
├── agents/
...
## 11. Future: Agent Engine Migration for Data and Intervention Agents
...
The long-term goal is to deploy all three agents to Agent Engine:
- Unified lifecycle management (versioning, rollback, scaling) through Agent Engine
- Built-in telemetry and monitoring via Agent Engine's observability stack
- Simplified authentication (intra-project Agent Engine calls use the caller's SA automatically)
- Potential to publish individual agents to Gemini Enterprise independently
- Consistent deployment pattern across all agents (`adk deploy agent_engine`)
...
### 11.4 What to Preserve

- Agent logic (`agent.py`), system prompts, and tool configurations are deployment-agnostic — they should work unchanged on either Cloud Run or Agent Engine
- The `agent_card.json` files describe agent capabilities and are used by A2A regardless of hosting
- The Cloud Run deployment scripts and Dockerfiles should be kept in the repo (or archived) as a reference for the Cloud Run approach