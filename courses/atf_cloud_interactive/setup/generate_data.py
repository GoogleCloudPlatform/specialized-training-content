#!/usr/bin/env python3
"""Creates BigQuery dataset/tables and generates synthetic Cymbal Meet data.

Deterministic output via seeded numpy RNG. See PRD section 3.4 for full spec.

Usage:
    python generate_data.py              # create tables, generate + load into BigQuery
    python generate_data.py --dry-run    # generate + print row counts only (no BQ)
...
# ---------------------------------------------------------------------------
# BigQuery table schemas (merged from create_bq_tables.py)
# ---------------------------------------------------------------------------
...
def create_dataset_and_tables(client: bigquery.Client, project_id: str):
    """Create BigQuery dataset and all tables. Idempotent (exists_ok=True)."""
...
def _telemetry_for_customer(rng: np.random.Generator, cust: dict,
                            weekdays: list[date],
                            time_slots: list[tuple[int, int]],
                            counter: int) -> tuple[list[dict], int]:
    """Generate telemetry rows for a single customer. Returns (rows, updated_counter)."""
    rows = []
    problem = cust["problem"]
...
def main():
    ...
    if args.dry_run:
        # Stream telemetry per-customer to count rows without OOM
        print("Generating device_telemetry (streaming per customer)...")
        telemetry_rng = np.random.default_rng(SEED_TELEMETRY)
        weekdays = get_weekdays(start, end)
        time_slots = [(h, m) for h in range(8, 18) for m in range(0, 60, 5)]
        telemetry_count = 0
        telemetry_counter = 0
        for cust in CUSTOMERS:
            cust_rows, telemetry_counter = _telemetry_for_customer(
                telemetry_rng, cust, weekdays, time_slots, telemetry_counter)
            telemetry_count += len(cust_rows)
        print(f"  {telemetry_count:,} rows")

        total = len(customers) + len(logins) + len(cal_events) + telemetry_count + len(calls)
    ...
    print(f"\nCreating dataset and tables in {dataset_ref}...")
    create_dataset_and_tables(client, project_id)
    ...
    # Stream telemetry per-customer to BigQuery to avoid OOM
    print("  device_telemetry: streaming per customer...")
    telemetry_rng = np.random.default_rng(SEED_TELEMETRY)
    weekdays = get_weekdays(start, end)
    time_slots = [(h, m) for h in range(8, 18) for m in range(0, 60, 5)]
    telemetry_count = 0
    ...
    for cust in CUSTOMERS:
        cust_rows, telemetry_counter = _telemetry_for_customer(
            telemetry_rng, cust, weekdays, time_slots, telemetry_counter)
        telemetry_count += len(cust_rows)
        load_to_bigquery(client, dataset_ref, "device_telemetry",
                         cust_rows, truncate=first)
        first = False
    print(f"  device_telemetry: {telemetry_count:,} rows loaded")
